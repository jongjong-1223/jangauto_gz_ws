"""로봇 URDF/SDF 기반 robot_state_publisher + 센서 프레임 static TF 발행(실기체용).

description_simul.launch.py와 static TF 값(물리적 장착 오프셋)은 동일 — 차이는
use_sim_time(False, 실제 시스템 클럭 사용)뿐. SDF/메시 등 모델 자산은 시뮬/실기체
공용이라 그대로 재사용한다.
"""
import os
...
def generate_launch_description():
    ...
    # 없으면 nav2 코스트맵의 메시지 필터가 계속 드롭한다(실행 확인됨, 시뮬 기준
    # — 실기체 카메라 드라이버도 동일 프레임명을 쓰도록 맞출 것).

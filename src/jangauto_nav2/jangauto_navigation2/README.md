# jangauto_navigation2

The `jangauto_navigation2` package is an example bringup system for Nav2 applications.

This is a very flexible example for nav2 bringup that can be modified for different maps/robots/hardware/worlds/etc. It is our expectation for an application specific robot system that you're mirroring `jangauto_navigation2` package and modifying it for your specific maps/robots/bringup needs. This is an applied and working demonstration for the default system bringup with many options that can be easily modified.
...
This minimal template ships `bringup_launch.py`, `navigation_launch.py`, `localization_launch.py`, `slam_launch.py`, and `rviz_launch.py`. Multi-robot / TurtleBot simulation launch files and example params/maps were removed to keep only what's needed for this robot.

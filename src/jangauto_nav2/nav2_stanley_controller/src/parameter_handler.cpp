  ...
  const double costmap_size_x)
{
  ...
  // Stanley 조향 법칙 파라미터 — 기본값은 로봇 풋프린트(길이 1.1m/폭 1.7m)
  // 기준으로 유도. wheelbase는 가상 전륜 오프셋(세로 길이의 절반),
  // min_turning_radius는 track_width/2(피벗 경계), max_steer_rad는
  // atan(wheelbase/min_turning_radius)로 이 둘과 기하학적으로 일치시킴.
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".stanley_wheelbase", rclcpp::ParameterValue(0.55));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".stanley_track_width", rclcpp::ParameterValue(1.7));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".stanley_min_turning_radius", rclcpp::ParameterValue(0.85));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".stanley_max_track_speed", rclcpp::ParameterValue(1.0));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".stanley_k", rclcpp::ParameterValue(0.5));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".stanley_k_soft", rclcpp::ParameterValue(1.0));
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".stanley_max_steer_rad", rclcpp::ParameterValue(0.574));
  ...
  node->get_parameter(plugin_name_ + ".stanley_wheelbase", params_.stanley_wheelbase);
  node->get_parameter(plugin_name_ + ".stanley_track_width", params_.stanley_track_width);
  node->get_parameter(
    plugin_name_ + ".stanley_min_turning_radius", params_.stanley_min_turning_radius);
  node->get_parameter(plugin_name_ + ".stanley_max_track_speed", params_.stanley_max_track_speed);
  node->get_parameter(plugin_name_ + ".stanley_k", params_.stanley_k);
  node->get_parameter(plugin_name_ + ".stanley_k_soft", params_.stanley_k_soft);
  node->get_parameter(plugin_name_ + ".stanley_max_steer_rad", params_.stanley_max_steer_rad);
  ...
  std::vector<rclcpp::Parameter> parameters)
{
  ...
  for (auto parameter : parameters) {
    ...
    if (type == ParameterType::PARAMETER_DOUBLE) {
      ...
      } else if (name == plugin_name_ + ".stanley_wheelbase") {
        params_.stanley_wheelbase = parameter.as_double();
      } else if (name == plugin_name_ + ".stanley_track_width") {
        params_.stanley_track_width = parameter.as_double();
      } else if (name == plugin_name_ + ".stanley_min_turning_radius") {
        params_.stanley_min_turning_radius = parameter.as_double();
      } else if (name == plugin_name_ + ".stanley_max_track_speed") {
        params_.stanley_max_track_speed = parameter.as_double();
      } else if (name == plugin_name_ + ".stanley_k") {
        params_.stanley_k = parameter.as_double();
      } else if (name == plugin_name_ + ".stanley_k_soft") {
        params_.stanley_k_soft = parameter.as_double();
      } else if (name == plugin_name_ + ".stanley_max_steer_rad") {
        params_.stanley_max_steer_rad = parameter.as_double();
      }

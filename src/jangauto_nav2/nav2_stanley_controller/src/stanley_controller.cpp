...
#include "nav2_stanley_controller/stanley_law.hpp"
...
std::unique_ptr<geometry_msgs::msg::PointStamped> StanleyController::createCarrotMsg(
  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  // use_fixed_curvature_lookahead: rotate_to_path 판단에 쓸 carrot만 별도
  // lookahead 지점으로 바꿈(시각화 목적 publish 포함) — 곡률 자체는 더 이상
  // 이 carrot에서 구하지 않고 아래 Stanley 조향각에서 유도한다.
  ...
  if (shouldRotateToGoalHeading(carrot_pose)) {
    ...
    // Stanley 조향각 계산 — transformed_plan은 이미 base_link 프레임(차량=원점,
    // yaw=0)이므로, 가상 전륜 위치를 (wheelbase, 0)으로 두고 그 지점 기준
    // 최근접 경로점을 찾는다(references/stanley_ref/stanley_module.c의
    // GPS→전륜축 투영과 동일한 역할, 좌표계만 로컬 프레임이라 더 단순함).
    const double front_axle_x = params_->stanley_wheelbase;
    auto nearest_it = std::min_element(
      transformed_plan.poses.begin(), transformed_plan.poses.end(),
      [&](const auto & a, const auto & b) {
        return hypot(a.pose.position.x - front_axle_x, a.pose.position.y) <
        hypot(b.pose.position.x - front_axle_x, b.pose.position.y);
      });
    // path_yaw(경로 접선 방향)는 pose.orientation이 아니라 인접 점 간 좌표로
    // 직접 계산한다 — nav2_straightline_planner는 중간 웨이포인트의
    // orientation을 전부 항등 쿼터니언으로 채우므로(실제 헤딩 아님, 마지막
    // 목표점만 예외) orientation을 그대로 쓰면 헤딩오차 항이 완전히
    // 틀어진다(지그재그 조향의 원인이었음).
    geometry_msgs::msg::Point tangent_from, tangent_to;
    if (std::next(nearest_it) != transformed_plan.poses.end()) {
      tangent_from = nearest_it->pose.position;
      tangent_to = std::next(nearest_it)->pose.position;
    } else {
      tangent_from = std::prev(nearest_it)->pose.position;
      tangent_to = nearest_it->pose.position;
    }
    const double path_yaw = std::atan2(
      tangent_to.y - tangent_from.y, tangent_to.x - tangent_from.x);
    const double psi_e = stanley_law::normalizeAngle(path_yaw);
    const double cross_track_error = stanley_law::crossTrackError(
      nearest_it->pose.position.x, nearest_it->pose.position.y, path_yaw,
      front_axle_x, 0.0);
    const double steer_rad = stanley_law::computeSteeringAngle(
      psi_e, cross_track_error, speed.linear.x,
      params_->stanley_k, params_->stanley_k_soft, params_->stanley_max_steer_rad);

    // applyConstraints()의 곡률 기반 감속에 쓸 곡률 = tan(δ)/wheelbase
    // (레퍼런스의 R_nom = wheelbase/tan(δ) 관계에서 curvature = 1/R_nom).
    const double regulation_curvature = std::tan(steer_rad) / params_->stanley_wheelbase;
    ...
    // δ와 (부호 포함) linear_vel을 궤도차량 좌/우 속도로 매핑한 뒤 다시
    // (linear.x, angular.z)로 합산 — min_turning_radius/max_track_speed
    // 클램프가 여기서 최종적으로 적용된다.
    const auto track_cmd = stanley_law::stanleyToTrack(
      steer_rad, linear_vel,
      params_->stanley_wheelbase, params_->stanley_track_width,
      params_->stanley_min_turning_radius, params_->stanley_max_track_speed);
    linear_vel = (track_cmd.v_left + track_cmd.v_right) / 2.0;
    angular_vel = (track_cmd.v_right - track_cmd.v_left) / params_->stanley_track_width;
  }
  ...
  {
    throw nav2_core::NoValidControl("StanleyController detected collision ahead!");
  }
...
double StanleyController::findVelocitySignChange(
  const nav_msgs::msg::Path & transformed_plan)
{
